...
Defines the app's own icon identity everywhere it can appear — replacing the placeholder, auto-generated Capacitor icon on Android (launcher, app drawer, recents, notifications) and the stock placeholder on the web build (favicon, in-app header) with the designed CouponKeeper mark, plus the store-listing asset used to publish it.
